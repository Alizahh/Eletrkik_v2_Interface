...
export function transformRoutesToTrade(args: GetQuoteArgs, data: QuoteData): TradeResult {
  ...
  const { gasUseEstimateUSD, blockNumber,gasPriceWei,gasUseEstimate } = data
  const totalGasEtherPrice = BigNumber.from(gasPriceWei).mul(BigNumber.from(gasUseEstimate)).mul(BigNumber.from(2000)).div(BigNumber.from("1000000000000000000")).toString()
